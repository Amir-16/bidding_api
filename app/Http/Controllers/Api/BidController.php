...
class BidController extends Controller
{
    ...
    {
        $bidRequestJson = $this->getSampleBidRequest();
        ...
        if (
            ...
            return json_encode(['error' => 'Invalid bid request parameters']);
        }
        ...
        $bidFloorCurrency = $bidRequest['imp'][0]['bidfloorcur'];
        ...
        if (empty($eligibleCampaigns)) {
            ...
            return json_encode($campaignResponse);
        }
    }


    private function getSampleBidRequest()
    {
        $jsonData = '{
    ...
    {
        // Sort eligible campaigns by bid price
        usort($eligibleCampaigns, function ($a, $b) {
            return $b['price'] <=> $a['price'];
        });

        // Return the first campaign (highest bid price)
